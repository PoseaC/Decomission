...
public abstract class BeatListener : MonoBehaviour
{
    ...
    public float beatSensitivityThreshold = 1.3f;
    ...
    public void HandleBeat(float beatValue, float secondAverage)
    {
        if (beatValue < secondAverage * beatSensitivityThreshold)
            return;
    ...
    IEnumerator Beat(bool rising)
    {
        ...
        while (timer < 1)
        {
            timer += interpolationSpeed * Time.deltaTime;
