...
public class Grapple : MonoBehaviour
{
    ...
    public GameObject cursorInactive;
    ...
    private void Update()
    {
        ...
        if (Physics.Raycast(mainCamera.position, mainCamera.forward, out RaycastHit hit, grappleDistance, whatIsGrappable))
        {
            ...
            if ((Input.GetButtonDown("Grapple") || (isAgent && AgentIO.output.Grapple == 1)) && !recalling)
            {
        ...
        if (Input.GetButtonUp("Grapple") || (isAgent && AgentIO.output.Grapple == 2))
        {
    ...
    IEnumerator DrawSineWave(bool throwing)
    {
        ...
        if (throwing)
        {
            ...
            katana.position += (endPosition - katana.position).normalized;
        ...
        while (localAmplitude > .05f)
        {
            ...
            if ((katana.position - endPosition).sqrMagnitude < 0.1f)
            {
                finishedFlag = true;
            }
    ...
    {
        cursorActive.SetActive(active);
        cursorInactive.SetActive(!active);
    }
}
